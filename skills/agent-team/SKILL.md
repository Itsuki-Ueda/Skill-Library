---
name: agent-team
description: チーム開発の共通手順。事前定義した実装・調査・レビュー担当へ契約で委譲し、統合検証まで行う。「/agent-team」「チームで開発」「分解して並列で」「ミッション」「Codex部隊に」「複数モジュールにまたがる開発」で使う。initは敷設、statusは読み取り専用、resumeは引継ぎ。使用中のハーネス用ラッパーを併読する。
---
...
- 通常ミッションの計画・コードは、それぞれの作成者とは別ベンダーの独立レビューを受ける。自己承認は禁止。**expressは親のdiff直読で合格可能**（同一ベンダーでも可、未達警告は不要）。
