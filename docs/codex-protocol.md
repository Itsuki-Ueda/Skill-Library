...
- 例外: `bash ~/.agents/skills/agent-team/scripts/cx-run.sh` はプロンプトをファイルから stdin に渡し、§3 の各形と同じ引数を組み立てるので Bash ツールで実行してよい。
...
## 7. 重大度タグとマッピング規則
- 重大度の基準の正本は `~/.agents/docs/review-severity.md`（P0=致命 / P1=重大 / P2=中 / P3=軽微・対応任意）。起こりにくく影響が中以下のもの、表記の揺れは P3。
- 文書レビュー・再レビューのプロンプトでは次を課す:
  「先頭に `VERDICT: pass|fail`（P0〜P2 相当が 1 件でもあれば fail）。指摘は `[P0]`〜`[P3]` タグ＋箇所付きで。重大度は `~/.agents/docs/review-severity.md` の基準に従い、P0〜P2 には起きる条件と影響を書く。無ければ『指摘なし』と明記。」
...
> 箇所（見出し/行）付きで簡潔に。重大度は `~/.agents/docs/review-severity.md` の基準に従い、P0〜P2 には起きる条件と影響を書く。
> 無ければ『指摘なし』と明記。対象を読んで評価: `<abs path>`
...
> 残る問題を `[P0]`/`[P1]`/`[P2]`/`[P3]` と箇所付きで簡潔に（重大度は `~/.agents/docs/review-severity.md`、P0〜P2 には起きる条件と影響）。先頭に `VERDICT: pass|fail`。無ければ『指摘なし』と明記。
