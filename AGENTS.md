...
- [確認 2026-09-26 | 根拠 docs/codex-model-comparison.md] Codex のモデル（sol / luna）・effort・レビューの回し方を変える判断や、Codex の使用量を減らす検討をするときは、先に `~/.agents/docs/codex-model-comparison.md`（実測の比較）を読む。
...
- 依頼された成果物、受入条件、終了地点を把握し、既存の指示で確定している事項は再確認しない。
- 許可された範囲で、実装、必要な検証、変更起因の失敗修正、再検証まで進める。
- 適用される承認・停止条件を守る。対処療法的な修正が最善の場合は、既存のエラー修正ルールに従って人間の判断を仰ぐ。
- 受入条件と必要な検証を満たし、依頼された終了地点に到達して完了とする。未実行事項と残作業は理由付きで報告する。
- 調査のみ・レビューのみ・変更案のみの依頼では、対象ファイルを変更せず、根拠を伴う結果の提示を終了地点とする。
