...
- `references/dispatch.md` で担当を選び、Allowed paths・依存の交差を確認する。**Allowed pathsが交差しなくても、検証が他タスクのパスを読み込むなら作業ツリーは競合資源**であり、レーンをworktreeで分ける（dispatch「並列実行の規律」）。直列化は競合資源を名指しする。集約ファイルは親の担当とする。
- レーンを分ける場合は、レーンごとの担当ファイル群・ブランチ・worktree・進行管理者をミッションの並列計画に書く。合流の順序と、合流後の再検証も計画に含める。
...
### [4] Dispatch
- 契約全文と役割スキルを名指しし、ラッパーの起動方法を使う。レーン運用ではレーンのworktreeを作業場所に指定する。session ID、成果物パス、作業場所、開始時刻を記録する。
...
- レーン運用ではレーンのブランチを計画の順序で合流させてから進む。未コミット変更ゼロを確認し、統合後HEADで `verify.post_change` と `verify.smoke` を実行する。実時間依存のテストは単独で再実行する。
...
- 撮影画像の残りを消す: `%TEMP%/agent-team-screens/<リポジトリ名>/` を削除する（タスクごとの削除漏れの受け皿）。
